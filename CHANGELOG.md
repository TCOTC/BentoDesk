...
- **微信文件拖拽**：可直接从微信聊天窗口拖拽文件和图片到盒子内。后台轮询线程可靠检测 OLE 拖拽时的鼠标释放，绕过 WinUI 3 对非 Chromium 拖拽源 Drop 事件不可靠的问题。
- **浏览器链接拖拽**：从浏览器拖拽图片或文件链接时，自动下载 URL 到临时文件并导入。
- **文件夹投放支持**：拖拽文件到盒子内的文件夹项目上时，文件会直接传输到该文件夹中。
...
- **静默回落修复**：`ClearTopMostPreservingForeground` 现在无条件调用 `BringWindowToFront(foreground)`，不再以 `wasTopMost` 门控。此前唤起的盒子使用瞬态置顶，门控恒为 false —— 回落状态变了但画面没变，导致下次按 F7 时"闪烁不收起"。
...
- **清空历史按钮**：搜索盒子底部新增清空按钮，可清除最近搜索和结果卡片，保留收藏项。
- **实时同步**：搜索弹窗中历史变化时，搜索盒子自动刷新。
- **移除自动推荐**：盒子主体现在只显示用户实际打开过的项目。自动生成的开始菜单应用快捷方式不再显示在盒子主体中。
- **简化占位符**：搜索占位符改为"搜索文件..."。
...
- **搜索盒子内容**：新增 `SearchWidgetContent` 和 `SearchWidgetContentAdapter`，搜索可作为盒子内容类型嵌入。
- **搜索设置分区**：新增独立的搜索设置页，支持快捷键配置、索引范围、结果数量和推荐开关。

#### 多语言扩展

- **三种新语言**：新增日语（ja-JP）、德语（de-DE）和巴西葡萄牙语（pt-BR）本地化，每种语言 1500+ 翻译条目，覆盖所有盒子、设置、引导、搜索、托盘菜单、对话框和状态消息。
...
- **批量动画驱动器**：新增 `WidgetTrayBatchAnimationDriver`，用于同步多盒子托盘显示/隐藏，消除单盒子计时漂移。
- **智能动画适配器**：新增 `SmartAnimationAdapter`，将自适应控制器与现有盒子动画路径桥接。
...
- **用户指南**：在 `docs/user-guide/` 下新增 10 章用户指南，涵盖快速开始、文件盒子、待办、随记、胶囊模式、叠放与 QuickLook、外观、备份、高级工作流和故障排除。
...
- **壁纸丢失**：防止 `WidgetLayerService` 盒子层级操作期间反复生成 `WorkerW` 窗口导致桌面壁纸丢失。
- **胶囊模式默认值**：将 `AppSettings` 初始值与 `ApplyDefaultPreferences` 对齐，确保新安装和全局重置产生一致的胶囊行为。
- **托盘菜单高度**：修复托盘右键菜单高度显示问题。
- **死快捷方式清理**：新增检测和移除无效 `.lnk` 和 `.url` 快捷方式（如已卸载的 Steam 游戏），此前这些快捷方式会在文件盒子中显示损坏图标。
...
- **音乐封面圆角**：修复音乐盒子封面在空状态和有值时圆角上下不对称的问题，通过动态计算 `CornerRadius` 解决。
...
- **手动排序改进**：文件盒子中任意拖拽操作现在自动切换到手动排序模式。后续排序模式点击恢复为自动排序。移除了显式的"手动排序"菜单选项。
...
- **胶囊模式**：盒子现在可以收起为紧凑桌面形态，并通过点击或标题区域悬停展开。收起后可显示关键信息、简要摘要或仅图标和标题，也可隐藏待办与随记正文等敏感内容。
- **稳定的收起与展开几何逻辑**：新增保持同宽和独立调整两种宽度关系，并根据屏幕可用区域从胶囊锚点向合适方向展开。胶囊与展开状态分别保存位置和尺寸，支持参考线与单盒覆盖；内容拖入时会临时展开，操作结束后不会永久改变两种状态。
- **胶囊组合排列**：胶囊可独立摆放，也可组成能够整体移动的组合栏。组合栏支持自定义顺序、悬浮或贴边位置、自动排列方向、间距调整，并能稳定恢复原来的自由布局位置。
- **文件自动叠放**：文件盒子可按文件类型或日期自动分组，不移动真实文件。自定义格式规则支持叠放名称、优先级、实时命中预览、形成数量、内部排序，以及未匹配文件保持散开或收入“其他”。
- **叠放交互与 QuickLook 兼容**：叠放在盒子内部散开展开和收回，并支持全选内容与复制路径。若 QuickLook 已经运行，在文件上按空格即可转交预览请求，不增加设置项、启动扫描或强制依赖。
...
- **可维护性与测试**：拆分体积过大的窗口、盒子、设置、待办、随记、音乐和天气类，并扩展附件、备份安全、设置迁移与搜索、胶囊边界与隐私、叠放、动画和窗口定位的回归测试。
...
- **待办体验重构**：将拥挤的单页操作改为卡片任务列表和盒子内完整详情页。详情标题支持多行输入并可拖动调整高度，提醒等元数据改为紧凑横排工具栏，任务卡片最多展示 10 行。
...
- **随记重新设计**：改为更舒展的记录列表和仅保留正文的盒子内详情编辑。支持纸张材质、添加/替换图片、缩略图、图片独立复制、固定状态反馈和创建时间显示开关。
- **随记排序与批量操作**：普通与固定列表均支持带动效的拖动排序，同时保留拖出复制。新增批量复制、删除、固定和纸张操作，修复详情返回后图片列表不及时刷新的问题。
- **盒子自适应布局**：最小尺寸由 200 x 200 降至 150 x 150。优化天气小尺寸和中等尺寸排版。音乐在小于 180 px 时切换为封面优先布局，保留歌名与播放控制，封面始终居中裁剪，复用跑马灯，并使用更慢且不展示空状态的封面过渡。
- **音乐功能精简**：完整移除频谱/节奏视觉、设置、ViewModel 状态和相关代码。统一最小与中等布局的实心播放图标、按钮尺寸及圆角矩形悬浮背景。
- **文件与右键菜单**：恢复流畅的竖排菜单并重新整理顺序，保留“打开”和“提取图片文字”，资源管理器命令改为“打开文件路径”，删除“显示更多选项”，删除命令固定在底部。文件盒子增加多选操作，菜单命令执行后立即关闭。
- **盒子标题编辑**：点击重命名后立即聚焦编辑；所有盒子支持双击标题进入编辑；点击标题剩余空白区域提交并退出。
- **动态层级修复**：重新整理托盘与快捷键唤起逻辑，确保全部盒子同时前置、不再闪烁一秒后置底，再次触发可一致隐藏，之后的前后台层级交还 Windows 管理。点击标题唤起全部盒子，点击内容只唤起当前盒子。
- **设置与重置行为**：全局重置保留语言、开机启动和功能盒子开关，正确重置随记创建时间显示与调整大小吸附，清除盒子标题栏独立覆盖值。功能盒子完整重置增加二次确认，标题菜单文案改为开关而非删除。
...
- **稳定性修复**：修复清空随记后添加入口消失、图文复制失败、创建时间开关不生效、列表与详情图片不同步、待办完成勾选对比度、待办内容偶发重叠、功能盒子重置不完整，以及多项托盘/快捷键层级边界问题。
...
- **天气盒子**：新增完整功能天气盒子，支持四种布局模式（迷你、紧凑、标准、详细），城市搜索（离线 `cities.json` 数据库 + Windows Location API）、自动定位、逐小时和每周预报、日出日落时间、紫外线指数、降水概率、湿度、风速、大气压、丰富皮肤背景和可配置刷新频率。标准视图在 250×215 及以上尺寸激活。逐小时温度列表支持横向滚动条和鼠标滚轮。周预报填满剩余空间，小尺寸下也可滚动。刷新时显示旋转加载动画并隐藏内容。
- **调整大小参考线**：新增 `ResizeGuideOverlayService`，在调整盒子大小时检测边缘对齐（8px 阈值），与其他盒子边缘和工作区边界对齐时显示强调色高亮条。已集成到 `WidgetWindow`、`QuickCaptureWidgetWindow` 和 `ContentWidgetWindow`。
- **随记修复**：盒子重置现在通过 `QuickCaptureService.ClearAsync()` 正确清理所有内部数据。修复首次切换 Tab 时空白显示的问题——数据现在立即显示，无需多次切换。为 `PlayItemsViewTransition` 添加 `ScheduleTransitionSafetyFallback` 安全兜底，提升内容切换可靠性。
- **默认设置统一**：默认外观改为云母材质、中等边框、大圆角——新安装、恢复默认和 `AppSettings` 初始值三处保持一致。全局重置（`ApplyDefaultPreferences`）现在恢复 `CustomAccentColor` 为 `#0078D4`、`FocusClickedWidgetOnRaise` 为 `false`。待办盒子单盒子重置现在恢复 `TodoReminderEnabled` 和 `TodoDefaultReminderOffsetMinutes`。天气盒子单盒子重置现在清除 `WeatherLatitude` 和 `WeatherLongitude`，避免定位行为不一致。
- **盒子标题字号**：功能盒子标题现在根据用户设置（`TextSize`/`IconSize`）动态计算 `TitleTextSize` 和 `TitleIconSize`，不再使用固定值。新增 `RefreshMetrics()` 方法在设置变化时更新标题度量。
- **内存泄漏修复**：`TodoWidgetViewModel` 实现 `IDisposable`，`TodoWidgetContentAdapter` 在盒子销毁时正确释放 ViewModel，防止内存泄漏。
- **删除盒子按钮**：所有类型盒子的"更多"菜单中均可删除盒子，不再限于文件盒子。修复 `ContentWidgetWindow` 更多菜单弹出位置。
- **设置页扩展**：外观设置新增独立的材质类型选择（云母 / 亚克力 / 纯色）和边框样式选择（无边框 / 细 / 中）。
- **盒子层级改进**：`WidgetLayerService` 新增 `SetWindowToDesktopLevel` 方法，不使用 `HWND_BOTTOM` 即可将窗口推至桌面层级，防止 Win+D 隐藏盒子。
...
- **代码重构 — WidgetManager**：将 `WidgetManager.cs`（3532 行）拆分为 4 个 partial 类：`WidgetManager.TrayAnimation.cs`（353 行，托盘显示/隐藏动画）、`WidgetManager.ZOrder.cs`（376 行，层级管理/鼠标钩子）、`WidgetManager.Storage.cs`（595 行，收纳管理/文件夹映射/孤立清理）、`WidgetManager.FeatureWidgets.cs`（889 行，音乐/天气/待办/随记功能盒子）。核心文件降至 1383 行。
...
- 将盒子托盘动画的固定 16ms `DispatcherQueueTimer` 替换为系统 VSync 驱动的 `CompositionTarget.Rendering`，动画帧率自动跟随显示器刷新率——60Hz 屏 60fps、144Hz 屏 144fps，消除高刷屏上的可见卡顿。
- 将毛玻璃背景的三次 `Task.Delay` 刷新替换为单个分阶段 `DispatcherQueueTimer`（80ms → 240ms → 580ms），减少线程池调度开销，所有刷新工作在 UI 线程定时器内完成。
- 移除盒子项目表面的空 `PointerMoved` 事件处理器，并为 `PointerEntered`/`PointerExited` 增加状态守卫，窗口关闭、动画运行或隐藏时跳过无意义的悬停状态计算。
- 音乐盒子失焦时停止进度、频谱和过渡定时器，激活时重新启动，消除不可见时的无效 CPU 占用。
...
- 优化待办盒子：截止时间保留时分秒精度，支持 Windows 原生提醒通知、逾期标识、已完成排序、单击复制、多选复制，以及更适合粘贴到聊天和 Markdown 的复制格式。
...
- 继续优化盒子操作图标：替换为 Fluent 图标形态，锁定状态有独立图标，紧凑标题下图标更小更清晰；重置标题样式时会清除单个盒子的覆盖值，让盒子重新跟随全局标题样式。
- 优化音乐盒子细节：系统音量浮窗改为横向，补齐浅色模式样式，支持点击空白区域关闭，滑杆更宽，封面氛围背景圆角会跟随盒子圆角设置。
- 在「文件盒子 -> 文件显示」中新增文件路径提示开关，可关闭鼠标移入文件时的完整路径提示。
...
- 在「设置 -> 常规」新增桌面盒子层级模式，用户可以保留现有动态层级，也可以切换到桌面固定层。
- 优化桌面固定层行为，让已显示的盒子在“显示桌面”/ Win+D 和显示环境刷新后继续回到桌面层。
- 设置左侧导航图标换成 Fluent 彩色图标，并继续调整设置页卡片间距和层级。
- 删除 ToggleSwitch 多余的开关文字，同时保留原生 WinUI 开关样式。
- 删除“盒子标题图标”和“更新状态”行里多余的装饰图标。
...
- 优化多屏和 DPI 场景下的盒子定位：显示器拓扑变化、缩放变化、外接屏插拔、1080p 更换 4K 显示器时，盒子会基于当前屏幕工作区重新恢复；隐藏盒子在重新显示前也会重新校验位置。
- 新增桌面盒子标题图标配置，支持彩色、面性单色、线性单色、隐藏和多语言文字标签模式。新安装和恢复默认设置都默认使用彩色图标。
- 文件盒子空白区域右键菜单新增标题样式选择，和标题栏菜单使用同一套行为。
- 设置页继续整理：将盒子标题图标偏好收进「外观 -> 窗口外观细节」。
- 对齐新用户默认值、恢复默认值和无效值兜底逻辑：动效和标题图标默认值保持一致，默认盒子动效统一为 `SlideFade`。
- 将随记默认视图归一化逻辑挪回随记设置归一化路径，避免后续维护混淆。
- 扩展自动化测试覆盖：设置默认值、盒子标题图标默认值、随记默认视图归一化，以及多屏感知的盒子定位。
...
- 完成 1.1.10 之后第一轮大规模盒子架构重构：文件盒子和功能盒子开始共享 `WidgetShell`、内容宿主、内容工厂、注册表、会话管理、窗口工厂和诊断路径，不再让每类盒子都维护一套孤立窗口实现。
- 建立功能盒子基础设施，用于承载待办、随记、音乐以及后续内容盒子：包括内容 Provider、盒子类型持久化、生命周期处理、位置管理、层级/会话行为和设置页集成。
- 新增待办盒子作为一等桌面盒子：支持本地存储、完成状态、筛选、行内编辑、全屏编辑、自定义结束时间，并补充存储、ViewModel 和内容适配层测试。
- 新增音乐盒子：接入 Windows 媒体会话，支持播放控制、播放模式切换、系统音量控制、自适应频谱、紧凑卡片布局、长歌名循环滚动和可选封面氛围取色。
- 将随记迁移到新的内容/盒子基础上，统一输入和编辑体验，优化最近内容刷新，给最近图片生成缩略图缓存，并减少重复预览加载。
- 统一文件盒子和功能盒子的外壳与编辑细节：标题栏尺寸、标题样式、行内编辑、全屏编辑层、悬停/按下状态、空状态、tooltip、操作按钮、分段控件以及浅色/深色图标行为。
- 按新架构重新整理设置页：功能盒子开关、外观分组、文件盒子显示、交互/全局快捷键、音乐频谱、随记偏好和本地化标签都做了重新归类和清理。
- 强化收纳目录与盒子生命周期维护：包括更安全的清理/恢复路径、默认收纳路径处理、盒子会话持久化，以及窗口诊断能力。
- 扩展自动化测试覆盖：新增内容工厂、盒子注册/会话/定位、内容窗口工厂、待办存储和 ViewModel、标题栏模式解析、功能盒子设置、收纳清理和随记缩略图相关测试。
...
- 修复随记最近内容过多时列表无法上下滚动的问题：列表区域现在会被限制在盒子剩余空间内，内部滚动可以正常工作。
- 新增第一阶段盒子架构重构路线文档，作为后续正式重构前的稳定基线。
...
- 修复盒子 z-order：盒子现在跟随 Windows 自然层级，不再被推到底层。新增 2 秒安全自动恢复定时器。
- 修复随记 z-order 与文件盒子的一致性：两者都使用 `ClearTopMostOnly`、`BringAllVisibleWidgetsToFront` 和 300ms 失焦守卫。
...
- 修复盒子动画：移除 SlideFade 和 ScaleSlide 效果的缩放，纯滑入。
- 修复全局快捷键（F7）打包后不工作。
- 修复删除盒子崩溃：`ApplyBackdropPreference` 加 `_isClosing` 守卫。
- 修复快捷键切换：`ShouldHideWidgetsForTrayToggle` 在盒子可见时返回隐藏。
...
- 优化盒子 z-order 可靠性：安全自动恢复定时器从 5 秒缩短到 2 秒。
...
- 修复盒子交互后可能卡在顶部的 z-order 问题：新增 5 秒安全定时器，如果盒子在无用户交互的情况下保持顶部状态，自动恢复到桌面层。
...
- 修复批量唤起盒子后点击外部窗口时，部分盒子仍留在浏览器上方的层级问题。
...
- 优化盒子动画性能：将每帧的 Win32 P/Invoke 透明度调用替换为 GPU 加速的 Visual.Opacity，缓存 Composition Visual，启用 Windows 原生贝塞尔缓动曲线。
- 精简动画设置：移除重复的方向特定效果，新增统一的"滑动方向"下拉框和"缓动强度"控制（无/轻微/标准/强烈）。方向下拉框在无滑动成分的效果下自动禁用。
- 修复文件盒子和随记盒子动画效果不一致的问题，两种盒子现在支持完全相同的效果和参数。
- 新增图片文件缩略图预览，替代原来的通用文件类型图标。
- 修复关闭"双击打开"后单击无法打开文件的问题。
- 修复右键点击文件时误触发单击打开而非弹出菜单的问题。
- 修复盒子框选逻辑吞掉 ItemClick 事件导致点击失效的问题。
- 移除"唤起后仅保留点击的盒子"设置，因不同盒子类型间 z-order 行为不一致。所有盒子现在统一显示和隐藏。
...
- 优化随记盒子 Tab 切换性能：为 item ViewModel 更新添加等值守卫，将 O(n²) 的集合同步替换为基于字典的 O(n) 查找，并缓存 Tab 和操作按钮的 Brush 以消除每次切换的对象分配。
- 新增"唤起后仅保留点击的盒子"设置。开启后点击一个盒子，其他盒子自动隐藏；关闭时（默认）所有盒子保持可见。
- 修复批量唤起盒子时的层级不一致问题：点击盒子时其他盒子不再跑到全屏应用后面，之前点过的盒子也不会意外留在前台。
- 统一文件盒子和随记盒子的层级处理逻辑，避免两者行为不一致。
...
- 新增设置内的拖拽异常诊断和一键修复，可清理 BentoDesk 的兼容性标记、启动项和快捷方式。如果 Win10/Win11 遇到文件拖不进盒子的问题，请先运行此修复。
- 优化资源管理器拖拽兼容，收纳盒子和映射盒子支持更多原生 shell 拖拽消息和旧格式兜底，并输出更完整的拖拽诊断日志。
- 修复盒子内排序稳定性，使用更接近 Windows 的自然名称排序，补充稳定兜底，并确保新加入文件按当前排序方式插入。
- 优化随记文本编辑：已保存文本双击进入随记内编辑；右键可选择“在记事本中编辑”，保存关闭后会同步回随记。
- 新安装和恢复默认设置时，托盘图标默认改为彩色。
- 优化新用户引导，首次安装启动后即标记为已完成，不会因为盒子为空而每次启动重复弹出；仍可在设置中手动打开。
...
- 新增盒子内排序方式：名称、大小、项目类型、修改日期，并支持按盒子保存排序状态和重复点击切换升序 / 降序。
- 优化盒子菜单，将标题栏的盒子管理操作和内容区的文件操作拆分得更清晰，包括视图切换、排序、粘贴、刷新和映射文件夹操作。
- 优化随记 Tab、标题栏按钮、悬浮按钮、复制反馈，以及与普通盒子一致的显示 / 恢复层级行为。
- 增强拖拽兼容、空盒子拖放、收纳 / 映射拖拽提示、层级恢复、图标加载重试和文件 / 文件夹重命名时的中文输入法支持。
...
- 优化资源管理器拖拽处理，文件盒子可兼容部分 link-style 拖拽操作，并按设置中的收纳动作安全处理。
- 优化拖拽悬浮提示，收纳盒子显示“收纳组件”，映射文件夹显示“映射文件夹”，目标更清楚。
- 优化随记复制反馈，移除每行内部气泡，统一改为底部居中的稳定 toast。
- 优化随记剪贴板写入，在 Windows 剪贴板被短暂占用时自动短间隔重试，减少第一次单击复制失败。
- 调整随记右上角按钮和记录悬浮按钮尺寸/样式，让更多、删除和记录操作更接近普通盒子控件。
- 修复 1.0.7 后发现的随记单击复制反馈、复制失败提示、映射盒子拖拽提示等细节问题。
...
- 优化托盘和全局快捷键行为，让文件盒子和随记按同一组逻辑统一置顶、隐藏和恢复。
- 新增轻量级 WidgetManager 层级恢复入口，菜单交互后不再由单个窗口自行置底，而是由管理器统一判断整组层级。
- 优化全屏应用场景：盒子可见但被外部应用遮挡时，F7 会重新置顶；同时增加快捷键钩子兜底，避免 Axure 等应用先消费快捷键。
- 优化盒子显示/隐藏动画，改为线性节奏、更短默认时长，并按整组相对屏幕位置计算滑出距离，减少遮挡和割裂感。
- 优化随记布局、悬浮按钮、Tab 切换、复制/打开行为、图片预览和长文本内联编辑体验。
- 随记支持单击复制并提示成功，双击按系统默认应用打开文本、链接或图片，不再用内部编辑弹框作为双击入口。
- 新增孤立收纳文件夹管理页，已移除盒子留下的收纳目录可在设置中恢复、打开、移回桌面或删除。
- 增强收纳盒子和映射盒子的稳定性，包括重名保护、异常文件夹恢复、映射文件夹改名同步、图标刷新稳定性和文件名显示修复。
...
- 新增随记功能盒子，用于本地保存文本、链接、截图和最近复制内容，功能可在设置中关闭。
- 随记支持记录、固定、最近三个视图，并加入悬停操作、紧凑搜索、拖出内容、图片缩略图和保存到文件盒子。
- 增强上传友好入口：收纳路径可固定到快速访问，可从托盘打开，并为文件选择器保留盒子文件夹快捷方式。
- 优化拖拽和剪贴板行为：文件拖拽优先保持文件格式，复制路径改为显式操作，并忽略 BentoDesk 自己写入剪贴板造成的最近记录污染。
- 优化文件盒子：支持资源管理器自定义图标刷新、文件后缀显示控制、快捷方式箭头设置归位，并补充迁移进度和结果反馈。
- 优化托盘和全局快捷键层级：盒子临时置顶后，只有点击其他应用才恢复；此状态下打开设置页也会临时置顶。
- 优化随记细节：增加当前视图搜索提示，保存到文件盒子后刷新并高亮目标文件，编辑弹窗适配小窗口，tab 和操作按钮布局更紧凑，并跟随 BentoDesk 主题色。
...
- 优化托盘左键逻辑：盒子临时置顶后，移动鼠标不会触发层级恢复，只有点击其他非 BentoDesk 窗口才会回到桌面层级。
- 增加多盒子托盘置顶后的二次确认，确保可见盒子能更稳定地被一起唤起。
- 优化托盘右键菜单定位，菜单会基于真实托盘图标位置弹出，并避开托盘图标点击区域。
- 增加毛玻璃背景自动刷新重试，在显示盒子、托盘唤起、主题和外观变化后恢复偶发的灰底问题。
- 重构设置窗口为左侧导航布局，将常规、外观、盒子布局、动画、文件与收纳、操作、重置与维护和关于分区展示。
...
- 在设置中新增可配置的盒子显示、隐藏动画效果和速度预设，并提供中文、英文标签。
- 重构托盘动画执行方式，使用更平滑的帧节奏、真实窗口移动和基于组合层的透明度、缩放过渡。
- 减少映射文件夹唤起时的盒子动画闪烁，并更稳定地恢复最终视觉状态。
- 优化托盘左键逻辑：被其他应用遮挡的桌面层级盒子会先被临时置顶，而不是立即隐藏。
- 优化托盘打开设置窗口时的临时置顶行为，设置窗口失焦后会清除置顶状态。
- 改进新建盒子、拖动盒子和文件夹选择器的前台窗口体验。
...
- 增加中文和英文本地化，覆盖盒子、设置、托盘菜单、新用户引导、对话框、提示、空状态和状态消息。
...
- 优化托盘行为，右键“显示全部盒子”会像左键点击托盘图标一样临时置顶盒子。
- 优化盒子显示、隐藏动画，统一为从右向左的动作，移除每个盒子的级联延迟，并减少映射盒子闪烁。
- 优化映射文件夹唤起行为，在窗口动画期间抑制重复的项目过渡。
- 优化浅色模式下的新用户引导和设置样式，包括文字对比度、界面颜色和当前步骤指示器形状。
- 优化盒子选中行为，在一个盒子中选择项目时会清除其他盒子的选择。
...
- 优化托盘唤起行为、盒子显示隐藏动画和临时前台行为。
- 优化默认设置、恢复默认值、外观实时预览和显示密度控制。
- 优化盒子文件交互，包括拖拽、剪切、重命名、删除确认和键盘快捷键。
