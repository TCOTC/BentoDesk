...
BentoDesk 当前是 WinUI 3 / .NET 10 桌面应用，发布方式为 Inno Setup 安装包。安装包支持覆盖安装，并会保留用户设置、盒子配置和收纳目录内容。因此第一阶段不建议做复杂的差分更新或热替换，优先采用“检查更新 -> 展示更新日志 -> 下载新版安装包 -> 校验 -> 启动安装器覆盖安装”的稳定方案。
...
```json
{
  ...
  "releaseNotes": {
    ...
    "items": [
      {
        "type": "feature",
        "text": "新增全局快捷键唤起盒子，默认 F7，可在设置中关闭或修改。"
      },
      {
        "type": "improvement",
