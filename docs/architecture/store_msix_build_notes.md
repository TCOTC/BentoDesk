# BentoDesk Store/MSIX 构建说明

本文档记录 Microsoft Store 技术分支的本地构建入口。当前 Direct/Inno 仍是默认发布通道，Store 包需要显式传入 `BentoDeskDistribution=Store`。
...
- 构建 `Release x64`
- 使用 `BentoDeskDistribution=Store`
- 跳过 `BentoDesk.Updater`
...
```powershell
.\scripts\build-store-msix.ps1 -SignPackage -PackageCertificateKeyFile "path\to\BentoDesk.pfx"
```

## 上架前必须替换

`src\BentoDesk\Package.appxmanifest` 里目前使用占位身份：

- `Identity Name="BentoDesk.Desktop"`
- `Publisher="CN=BentoDesk"`
- `PublisherDisplayName="BentoDesk"`
...
5. 确认包内没有 `BentoDesk.Updater.*`、`donation-*` 或 `store-assets-html/` 这类非 Store 包资源。
