...
对音乐、系统监控这类偏展示型格子来说，固定 46px 标题栏会占用明显空间，且“更多/删除/标题”不是主要内容。它们更适合内容铺满，管理按钮以悬浮方式出现，甚至允许隐藏。

所以后续应引入“格子外壳模式”，而不是为音乐、监控重新写专用窗口。
...
后续新增音乐、系统监控时，应该优先复用这套外壳模式。这样既能保留 BentoDesk 的统一行为，也能让不同类型格子获得适合自己的视觉密度。
