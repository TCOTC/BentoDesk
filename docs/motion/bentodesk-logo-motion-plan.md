...
- `bentodesk-logo-static.svg`: 当前 `src/BentoDesk/Assets/bentodesk.svg` 的复制版，作为视觉基准。
- `bentodesk-logo-motion-ready.svg`: 同一静态图，但给三层 path 增加了稳定 id：`layer-back`, `layer-middle`, `layer-front`。
...
3. `bentodesk-motion-03-focus-sheen.svg`
...
- 减少动画用户应直接显示 `bentodesk-logo-static.svg` 的最终帧。
