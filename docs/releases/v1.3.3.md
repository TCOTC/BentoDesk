...
BentoDesk 1.3.3 是一个改进与修复版本。新增微信文件和浏览器链接拖拽支持，引入叠放组管理（重命名、排序、取消折叠），添加 MSN 天气作为可选数据源并支持自动切换备用源，修复 F7 唤起/回落的层级可靠性问题，并改进搜索盒子体验。
...
- **静默回落修复（方案 A）**：`ClearTopMostPreservingForeground` 现在无条件调用 `BringWindowToFront(foreground)`，不再以 `wasTopMost` 门控。此前唤起的盒子使用瞬态置顶（TOPMOST→NOTOPMOST），门控恒为 false —— 回落状态变了但画面没变，导致下次按 F7 时"闪烁不收起"。
...
- **实时同步**：搜索弹窗中历史变化时，搜索盒子自动刷新，无需重新打开。
- **移除自动推荐**：盒子主体现在只显示用户实际打开过的项目（最近结果）。自动生成的开始菜单应用快捷方式不再显示在盒子主体中，仍可在搜索弹窗中查看。
- **简化占位符**：搜索占位符改为"搜索文件..."。
