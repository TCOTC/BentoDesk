...
【头图占位：BentoDesk 桌面全景，建议使用干净的深色或浅色桌面，同时展示文件、待办、随记和音乐格子】
...
BentoDesk 最基础的东西，是一个个可以独立摆放和调整大小的文件格子。

其中一种叫“收纳格子”。它背后对应一个真实文件夹，把文件拖进去，就会移动到对应目录。你在资源管理器里仍然能看到这些文件，也可以正常复制、重命名、删除和打开。BentoDesk 不会把文件塞进自己的数据库，更不会把它们变成只有软件自己认识的格式。
...
如果每次都要打开一个完整应用，处理成本反而有点高。所以 BentoDesk 后来增加了待办、随记和音乐几类功能格子。
...
音乐格子只连接 Windows 的系统媒体会话，用来显示封面、歌名和常用播放控制。
...
BentoDesk 默认采用动态层级。通过托盘或快捷键唤起时，所有格子会回到前台；之后不再强行霸占最上层，而是把窗口关系重新交给 Windows 管理。
...
另外需要特别说明：把文件拖入“收纳格子”时，文件仍留在用户桌面目录，格子只记录归属；从下载等非桌面位置拖入时，会先按设置复制或移动到桌面。如果只想查看现有目录而不改变位置，应该使用“文件夹映射”。
...
文件有地方放，临时内容有地方记，待办能看见，音乐可以顺手处理。需要的时候把格子叫出来，处理完之后，再把注意力交还给正在做的事情。
