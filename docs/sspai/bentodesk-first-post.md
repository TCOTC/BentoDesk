...
如果每次都要打开一个完整应用，处理成本反而有点高。所以 BentoDesk 后来增加了待办、随记、搜索和音乐几类功能格子。
...
音乐格子只连接 Windows 的系统媒体会话，用来显示封面、歌名和常用播放控制；搜索格子则把常用检索入口放在桌面上，方便快速找回文件。
...
它不适合需要云同步、多人协作、复杂项目管理或完整知识库的用户。待办、随记、搜索和音乐都刻意保留了边界，它们不会替代专业应用。
...
文件有地方放，临时内容有地方记，待办能看见，搜索和音乐可以顺手处理。需要的时候把格子叫出来，处理完之后，再把注意力交还给正在做的事情。
