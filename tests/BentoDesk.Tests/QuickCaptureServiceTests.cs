...
public sealed class QuickCaptureServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task AddRecentClipboardItemAsync_IgnoresTextWrittenByBentoDesk()
    {
        var service = CreateService();

        service.MarkClipboardTextWrittenByBentoDesk("copied from bentodesk");
        var item = await service.AddRecentClipboardItemAsync("copied from bentodesk", QuickCaptureService.DefaultRecentLimit);
        var data = await service.GetDataAsync();
