...
{
    private readonly string _tempRoot = Path.Combine(Path.GetTempPath(), "BentoDesk.Tests", Guid.NewGuid().ToString("N"));
    ...
    public async Task CheckForUpdatesAsync_ReadsManifestAndDetectsUpdate()
    {
        using var httpClient = CreateHttpClient(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(
                """
                {
                  "schemaVersion": 1,
                  "version": "1.2.2",
                  "downloadUrl": "https://github.com/TCOTC/BentoDesk/releases/download/v1.2.2/BentoDesk_Setup_1.2.2_x64.exe",
        ...
        var service = new AppUpdateService(httpClient, "https://github.com/TCOTC/BentoDesk/releases/latest", _tempRoot);
    ...
    public async Task CheckForUpdatesAsync_FallsBackToGitHubReleaseWhenManifestUnavailable()
    {
        using var httpClient = CreateHttpClient(request =>
        {
            ...
            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent(
                    ...
                    {
                      "tag_name": "v1.2.4",
                      "html_url": "https://github.com/TCOTC/BentoDesk/releases/tag/v1.2.4",
                      "assets": [
                        {
                          "name": "BentoDesk_Setup_1.2.4_x64.exe",
                          "browser_download_url": "https://github.com/TCOTC/BentoDesk/releases/download/v1.2.4/BentoDesk_Setup_1.2.4_x64.exe",
                          "size": 23423211,
        ...
        Assert.Equal("https://github.com/TCOTC/BentoDesk/releases/download/v1.2.4/BentoDesk_Setup_1.2.4_x64.exe", result.Manifest.DownloadUrl);
        ...
        Assert.Equal("https://github.com/TCOTC/BentoDesk/releases/tag/v1.2.4", result.Manifest.ReleaseNotesUrl);
    }
    ...
    public async Task CheckForUpdatesAsync_FallsBackToGitHubShaAssetWhenDigestMissing()
    {
        using var httpClient = CreateHttpClient(request =>
        {
            ...
            if (url.EndsWith(".sha256", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Content = new StringContent(
                        "8ECB3092AE5BD6883F8A75BBEA03D9800251E0C23FDBE1BF4D91FD3A62565561  BentoDesk_Setup_1.2.4_x64.exe",
                        Encoding.UTF8,
                        "text/plain")
                };
            }
                ...
                Content = new StringContent(
                    """
                    {
                      ...
                      "assets": [
                        ...
                        {
                          "name": "BentoDesk_Setup_1.2.4_x64.exe.sha256",
                          "browser_download_url": "https://github.com/TCOTC/BentoDesk/releases/download/v1.2.4/BentoDesk_Setup_1.2.4_x64.exe.sha256",
                          "size": 95
                        }
                      ]
                    }
    ...
    public async Task DownloadUpdateAsync_RejectsInstallerWithoutHash()
    {
        ...
        {
            Version = "1.2.2",
            DownloadUrl = "https://example.com/BentoDesk_Setup_1.2.2_x64.exe"
        });
    ...
    {
        byte[] payload = Encoding.UTF8.GetBytes("bentodesk-installer");
    ...
    public async Task PrepareDetachedUpdaterHelper_CopiesUpdaterOutsideAppDirectory()
    {
        ...
        string sourceExe = Path.Combine(appDirectory, "BentoDesk.Updater.exe");
        string sourceRuntimeConfig = Path.Combine(appDirectory, "BentoDesk.Updater.runtimeconfig.json");
        ...
        Assert.True(File.Exists(Path.Combine(Path.GetDirectoryName(detachedExe)!, "BentoDesk.Updater.runtimeconfig.json")));
    }
