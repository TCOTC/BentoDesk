...
public sealed class QuickCaptureClipboardServiceTests : IDisposable
{
    ...
    {
        BentoDeskClipboardWriteScope.ClearForTesting();
    ...
    [Fact]
    public async Task CaptureCurrentForTestingAsync_IgnoresBentoDeskWrittenText()
    {
        var settingsService = await CreateLoadedSettingsServiceAsync();
        EnableClipboardCapture(settingsService);
        var quickCaptureService = CreateQuickCaptureService();
        var reader = new FakeClipboardReader { Text = "copied inside BentoDesk" };
        using var service = new QuickCaptureClipboardService(settingsService, quickCaptureService, reader);

        BentoDeskClipboardWriteScope.MarkWrite(text: "copied inside BentoDesk");
        await service.CaptureCurrentForTestingAsync();

        var data = await quickCaptureService.GetDataAsync();
        Assert.Empty(data.RecentItems);
        Assert.Equal("ignored:bentodesk-write", service.GetDiagnostics().LastReason);
    }

    [Fact]
    public async Task CaptureCurrentForTestingAsync_IgnoresBentoDeskWrittenImage()
    {
        var settingsService = await CreateLoadedSettingsServiceAsync();
        EnableClipboardCapture(settingsService);
        var quickCaptureService = CreateQuickCaptureService();
        ...
        BentoDeskClipboardWriteScope.MarkWrite(hasImage: true);
        await service.CaptureCurrentForTestingAsync();
    ...
    [Fact]
    public async Task CaptureCurrentForTestingAsync_IgnoresBentoDeskWrittenPathText()
    {
        var settingsService = await CreateLoadedSettingsServiceAsync();
        EnableClipboardCapture(settingsService);
        var quickCaptureService = CreateQuickCaptureService();
        ...
        BentoDeskClipboardWriteScope.MarkWrite(paths: [filePath]);
        await service.CaptureCurrentForTestingAsync();
