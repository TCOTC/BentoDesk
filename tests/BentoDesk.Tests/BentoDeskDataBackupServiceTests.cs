...
public sealed class BentoDeskDataBackupServiceTests : IDisposable
{
    private readonly string _tempRoot;
    private readonly string _appDataRoot;
    private readonly string _exportRoot;

    public BentoDeskDataBackupServiceTests()
    {
    ...
    public async Task CreateAutomaticSnapshotIfDueAsync_CreatesOnlyOneSnapshotPerDay()
    {
        ...
        Assert.Single(Directory.EnumerateFiles(service.AutomaticSnapshotDirectory, "BentoDesk-Auto-*.zip"));
        Assert.False(Directory.Exists(service.BackupSnapshotStagingDirectory));
    }
    ...
    public async Task SnapshotInventory_ReportsUnreadableEntriesAndAllowsManagedDeletion()
    {
        ...
        string unreadablePath = Path.Combine(service.PreRestoreBackupDirectory, "BentoDesk-PreRestore-broken.zip");
        await File.WriteAllTextAsync(unreadablePath, "not a zip archive");

        IReadOnlyList<BentoDeskBackupSnapshotInfo> snapshots = await service.GetSnapshotInventoryAsync();
    ...
    public async Task ApplyPendingRestoreAsync_FailureClearsPendingRestoreAndPreservesCurrentData()
    {
        ...
        string archivePath = await new BentoDeskDataBackupService(sourceRoot).ExportBackupAsync(_exportRoot);
        ...
        BentoDeskRestoreApplyResult result = await service.ApplyPendingRestoreAsync();
