...
procedure DetectBentoDeskDependencies;
begin
  ...
  Log('BentoDesk dependency check: dotnet10Missing=' + IntToStr(Integer(ShouldInstallDotNetRuntime)));
  Log('BentoDesk dependency check: windowsAppRuntimeMissing=' + IntToStr(Integer(ShouldInstallWindowsAppRuntime)));
end;
...
function DownloadBentoDeskDependencies: Boolean;
var
  ErrorMessage: string;
begin
  Result := True;
...
begin
  ...
  begin
    SuppressibleMsgBox(
      ...
      'Please confirm this Windows version is supported, or install the dependency manually and run BentoDesk setup again.',
      mbCriticalError,
      MB_OK,
      IDOK);
    Exit;
...
function InstallBentoDeskDependencies(var NeedsRestart: Boolean): Boolean;
var
  Step: Integer;
  StepCount: Integer;
begin
...
begin
  ...
  NeedsRestart := False;
  DetectBentoDeskDependencies;

  if not DownloadBentoDeskDependencies then
  begin
    Result := False;
    Exit;
  end;

  if not InstallBentoDeskDependencies(NeedsRestart) then
  begin
    Result := False;
    Exit;
  end;
