...
namespace BentoDesk.Controls.WidgetContents;
...
public sealed partial class TodoWidgetContent
{
    ...
    private void ColorFilterButton_DragStarting(UIElement sender, DragStartingEventArgs e)
    {
        ...
        BentoDeskDragData.SetTodoColorMarker(e.Data, colorMarker);
    ...
    private void TodoItem_DragOver(object sender, DragEventArgs e)
    {
        ...
        if (e.DataView.Contains(BentoDeskDragData.TodoColorMarkerFormat))
        {
            e.Handled = true;
        ...
        if (BentoDeskDragData.HasDroppedFiles(e.DataView))
        {
            e.Handled = true;
    ...
    {
        if (e.DataView.Contains(BentoDeskDragData.TodoColorMarkerFormat) ||
            BentoDeskDragData.HasDroppedFiles(e.DataView))
        {
            e.Handled = true;
    ...
    private async void TodoItem_Drop(object sender, DragEventArgs e)
    {
        ...
        if (e.DataView.Contains(BentoDeskDragData.TodoColorMarkerFormat))
        {
            ...
            string? colorMarker = TodoItem.NormalizeColorMarker(
                await BentoDeskDragData.TryGetTodoColorMarkerAsync(e.DataView));
        ...
        try
        {
            using DroppedFileBatch batch = await BentoDeskDragData.TryGetDroppedFilesAsync(e.DataView);
    ...
    private void TodoListView_DragItemsStarting(object sender, DragItemsStartingEventArgs e)
    {
            ...
            selectedItems.Contains(draggedItem))
        {
            ...
            BentoDeskDragData.SetText(e.Data, text, BentoDeskDragData.SourceTodo);
    ...
    private async Task HandleExternalTextDragOverAsync(DragEventArgs e)
    {
        ...
        try
        {
            e.AcceptedOperation = BentoDeskDragData.HasDroppedFiles(e.DataView) ||
    ...
    {
        return BentoDeskDragData.HasDroppedFiles(dataView) ||
    ...
    internal async Task<bool> ImportExternalDropAsync(DataPackageView dataView)
    {
        try
        {
            if (BentoDeskDragData.HasDroppedFiles(dataView))
            {
                using DroppedFileBatch batch = await BentoDeskDragData.TryGetDroppedFilesAsync(dataView);
    ...
    private static async Task<string?> TryGetDroppedTodoTextAsync(DataPackageView dataView)
    {
        ...
        string? text = await BentoDeskDragData.TryGetTextAsync(dataView);
        if (string.IsNullOrWhiteSpace(text))
        {
            return null;
        }
