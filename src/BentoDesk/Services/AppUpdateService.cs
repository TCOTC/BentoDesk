...
{
    public const string DefaultManifestUrl = "https://github.com/TCOTC/BentoDesk/releases/latest/download/stable.json";
    public const string DefaultGitHubLatestReleaseApiUrl = "https://api.github.com/repos/TCOTC/BentoDesk/releases/latest";
    public const string DefaultManualDownloadUrl = "https://github.com/TCOTC/BentoDesk/releases";
        ...
        string? githubLatestReleaseApiUrl = null)
    {
        ...
        _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("BentoDesk", GetCurrentAppVersion()));
        ...
        _updateRootPath = string.IsNullOrWhiteSpace(updateRootPath)
            ? BentoDeskDataPathService.Current.UpdatesDirectory
            : updateRootPath;
    }
    ...
    public AppUpdateInstallResult StartInstallerHelper(string installerPath, bool silent = true)
    {
        ...
        {
            appPath = Path.Combine(AppContext.BaseDirectory, "BentoDesk.exe");
        }
        ...
        try
        {
            ...
            {
                return AppUpdateInstallResult.Failed("BentoDesk.Updater.exe was not found in the application directory.");
            }
    ...
    {
        string sourceHelperPath = Path.Combine(appDirectory, "BentoDesk.Updater.exe");
        ...
        foreach (var sourcePath in Directory.EnumerateFiles(appDirectory, "BentoDesk.Updater.*", SearchOption.TopDirectoryOnly))
        {
            string targetPath = Path.Combine(helperDirectory, Path.GetFileName(sourcePath));
            File.Copy(sourcePath, targetPath, overwrite: true);
        }

        return Path.Combine(helperDirectory, "BentoDesk.Updater.exe");
    }
    ...
    private static string GetInstallerFileName(Uri downloadUri, string version)
    {
        ...
        return string.IsNullOrWhiteSpace(fileName)
            ? $"BentoDesk_Setup_{SanitizePathSegment(version)}_x64.exe"
    ...
    {
        return asset.Name.StartsWith("BentoDesk_Setup_", StringComparison.OrdinalIgnoreCase) &&
