...
public sealed partial class WidgetManager
{
    ...
    private void DeleteMappedWidgetShortcut(string shortcutPath, string widgetId)
    {
        if (string.IsNullOrWhiteSpace(shortcutPath) ||
            ...
            !IsBentoDeskMappedWidgetShortcut(shortcutPath, widgetId))
        {
            return;
        }
    ...
    private void RemoveStaleMappedWidgetShortcuts(string rootPath, ISet<string> activeWidgetIds)
    {
        ...
        {
            string? widgetId = GetBentoDeskMappedWidgetShortcutId(shortcutPath);
    ...
    private string GetExistingMappedWidgetShortcutPath(WidgetConfig config, string rootPath)
    {
        ...
        return Directory.EnumerateFiles(rootPath, "*.lnk", SearchOption.TopDirectoryOnly)
            .FirstOrDefault(path => IsBentoDeskMappedWidgetShortcut(path, config.Id)) ?? string.Empty;
    }
    ...
    private bool CanUseMappedShortcutPath(string shortcutPath, string widgetId)
    {
        ...
        return File.Exists(shortcutPath) && IsBentoDeskMappedWidgetShortcut(shortcutPath, widgetId);
    }

    private static bool IsBentoDeskMappedWidgetShortcut(string shortcutPath, string widgetId)
    {
        return string.Equals(
            GetBentoDeskMappedWidgetShortcutId(shortcutPath),
            widgetId,
            StringComparison.Ordinal);
    }

    private static string? GetBentoDeskMappedWidgetShortcutId(string shortcutPath)
    {
