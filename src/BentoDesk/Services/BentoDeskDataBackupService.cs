...
public sealed class BentoDeskDataBackupService
{
    ...
    public BentoDeskDataBackupService()
        : this(BentoDeskDataPathService.Current.RootPath)
    {
    }

    internal BentoDeskDataBackupService(string rootPath)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            string? latestSnapshot = Directory
                .EnumerateFiles(AutomaticSnapshotDirectory, "BentoDesk-Auto-*.zip")
            ...
            string snapshotPath = GetAvailableArchivePath(
                ...
                $"BentoDesk-Auto-{DateTime.Now:yyyyMMdd-HHmmss}.zip");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (!HasBackupSourceData())
            {
                throw new InvalidOperationException("BentoDesk data directory is empty.");
            }

            Directory.CreateDirectory(destinationDirectory);
            string backupPath = GetAvailableArchivePath(
                destinationDirectory,
                $"BentoDesk-Backup-{DateTime.Now:yyyyMMdd-HHmmss}.zip");
    ...
    public async Task<BentoDeskRestorePreparation> PrepareRestoreAsync(
        string archivePath,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new FileNotFoundException("The selected BentoDesk backup does not exist.", archivePath);
        }
        ...
        try
        {
            ...
            return new BentoDeskRestorePreparation(
                archiveInfo.Manifest.CreatedAtUtc,
                archiveInfo.Manifest.AppVersion,
    ...
    internal async Task<BentoDeskRestoreApplyResult> ApplyPendingRestoreAsync(
        CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        ...
        try
        {
            ...
            {
                return BentoDeskRestoreApplyResult.NoPendingRestore;
            }
            ...
            if (HasBackupSourceData())
            {
                ...
                string preRestorePath = GetAvailableArchivePath(
                    ...
                    $"BentoDesk-PreRestore-{DateTime.Now:yyyyMMdd-HHmmss}.zip");
            ...
            return new BentoDeskRestoreApplyResult(true, true, null);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
        ...
        catch (Exception ex)
        {
            ...
            DeletePendingRestoreCore();
            return new BentoDeskRestoreApplyResult(true, false, ex.Message);
        }
        finally
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        BentoDeskBackupManifest manifest;
        await using (Stream manifestStream = manifestEntry.Open())
        {
            manifest = await JsonSerializer.DeserializeAsync<BentoDeskBackupManifest>(
                           manifestStream,
                           s_jsonOptions,
                           cancellationToken) ??
            ...
            manifest.SchemaVersion > BackupSchemaVersion)
        {
            throw new InvalidDataException(
                $"Unsupported BentoDesk backup schema version {manifest.SchemaVersion}.");
        }

        if (IsBackupFromNewerApp(manifest.AppVersion))
        {
            throw new InvalidDataException(
                $"This backup was created by newer BentoDesk version {manifest.AppVersion}.");
        }
        ...
        var extractedFiles = new Dictionary<string, BentoDeskBackupFileManifest>(StringComparer.OrdinalIgnoreCase);
        int fileCount = 0;
        ...
        foreach (ZipArchiveEntry entry in archive.Entries)
        {
            ...
            extractedFiles[relativePath] = new BentoDeskBackupFileManifest(
                relativePath,
                extractedLength,
                sha256);
        }
    ...
    public async Task<IReadOnlyList<BentoDeskBackupSnapshotInfo>> GetSnapshotInventoryAsync(
        CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try
        {
            ...
            var snapshots = new List<BentoDeskBackupSnapshotInfo>(paths.Count);
            foreach ((string path, string kind) in paths)
            {
                ...
                snapshots.Add(new BentoDeskBackupSnapshotInfo(
                    path,
                    kind,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("The selected backup snapshot is not managed by BentoDesk.");
        }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            await using Stream manifestStream = manifestEntry.Open();
            BentoDeskBackupManifest? manifest = await JsonSerializer.DeserializeAsync<BentoDeskBackupManifest>(
                manifestStream,
                s_jsonOptions,
                cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!File.Exists(settingsPath))
        {
            throw new InvalidOperationException("BentoDesk settings are not available for backup.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
                             ...
                             useAsync: true))
            {
                ...
                {
                    var fileManifest = new List<BentoDeskBackupFileManifest>(sourceFiles.Length);
                    foreach ((string sourcePath, string relativePath) in sourceFiles)
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        ...
                        (long length, string sha256) = await CopyAndHashAsync(
                            ...
                            destination,
                            cancellationToken);
                        fileManifest.Add(new BentoDeskBackupFileManifest(relativePath, length, sha256));
                    }

                    var manifest = new BentoDeskBackupManifest(
                        BackupSchemaVersion,
                        backupKind,
                        DateTimeOffset.UtcNow,
                        typeof(BentoDeskDataBackupService).Assembly.GetName().Version?.ToString() ?? "unknown",
                        DataDirectory,
                        fileManifest);
        ...
        CancellationToken cancellationToken)
    {
        ...
        TryDeleteFile(destinationPath);
        throw new IOException($"BentoDesk data file changed while creating a backup snapshot: '{sourcePath}'.");
    }
    ...
    {
        foreach (string obsoletePath in Directory
                     .EnumerateFiles(PreRestoreBackupDirectory, "BentoDesk-PreRestore-*.zip")
                     .OrderByDescending(File.GetLastWriteTimeUtc)
    ...
    private static void ValidateIntegrityManifest(
        IReadOnlyList<BentoDeskBackupFileManifest>? expectedFiles,
        IReadOnlyDictionary<string, BentoDeskBackupFileManifest> extractedFiles)
    {
        ...
        var expectedByPath = new Dictionary<string, BentoDeskBackupFileManifest>(StringComparer.OrdinalIgnoreCase);
        foreach (BentoDeskBackupFileManifest expected in expectedFiles)
        {
            if (expected is null ||
        ...
        foreach ((string path, BentoDeskBackupFileManifest actual) in extractedFiles)
        {
            if (!expectedByPath.TryGetValue(path, out BentoDeskBackupFileManifest? expected) ||
    ...
    {
        string? currentVersion = typeof(BentoDeskDataBackupService).Assembly.GetName().Version?.ToString();
    ...
    private sealed record BentoDeskBackupManifest(
        int SchemaVersion,
        string Kind,
        DateTimeOffset CreatedAtUtc,
        string AppVersion,
        string? SourceDataPath = null,
        IReadOnlyList<BentoDeskBackupFileManifest>? Files = null);

    private sealed record BentoDeskBackupFileManifest(
        string Path,
        long Length,
        string Sha256);
...
public sealed record BentoDeskBackupSnapshotInfo(
    string Path,
    string Kind,
    DateTimeOffset CreatedAtUtc,
    long SizeBytes,
...
public sealed record BentoDeskRestorePreparation(
    DateTimeOffset BackupCreatedAtUtc,
    string AppVersion,
    int FileCount,
    long TotalUncompressedBytes,
...
internal sealed record BentoDeskRestoreApplyResult(
    ...
    string? ErrorMessage)
{
    public static BentoDeskRestoreApplyResult NoPendingRestore { get; } = new(false, true, null);
}
