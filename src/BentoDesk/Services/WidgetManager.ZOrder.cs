...
public sealed partial class WidgetManager
{
    ...
    // position at the moment of pressing. If the press is outside BentoDesk /
    ...
    private void TryRestoreRaisedWidgetsAfterInteraction(string reason, long generation)
    {
        ...
        if (IsBentoDeskForegroundWindow(foreground))
        {
            _hasBentoDeskForegroundSinceRaise = true;
        ...
        if (!_hasBentoDeskForegroundSinceRaise)
        {
            ...
            //
            // Primary signal: the foreground window CHANGED to a different non-BentoDesk
        ...
        App.Log($"[TrayBatch] RaisedState released reason={reason}-bentodesk-leave foreground=0x{foreground.ToInt64():X}");
        RestoreRaisedWidgetsToDesktopLayer();
    }
    ...
    private void StartTrayLayerRestoreMonitor(bool hasRaisedWidgets)
    {
        ...
        _hasBentoDeskForegroundSinceRaise = IsForegroundBentoDeskWindow();
    ...
    private void TrayMouseSamplerTimer_Tick(DispatcherQueueTimer sender, object args)
    {
        ...
        if (isDown && !_lastMouseButtonsDown)
        {
            ...
            if (!IsPointerOverBentoDeskWindow(cursor) &&
    ...
    private static bool IsPointerOverBentoDeskWindow(Win32Helper.POINT? cursor)
    {
        if (!cursor.HasValue)
        {
            return false;
    ...
    private static bool IsForegroundBentoDeskWindow()
    {
        IntPtr foregroundWindow = Win32Helper.GetForegroundWindow();
        return IsBentoDeskForegroundWindow(foregroundWindow);
    }

    private static bool IsBentoDeskForegroundWindow(IntPtr foregroundWindow)
    {
        return foregroundWindow != IntPtr.Zero &&
               App.Current.IsBentoDeskWindow(foregroundWindow);
    }
