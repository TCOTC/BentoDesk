...
public static class DragDropPermissionService
{
    ...
    private const string AppName = "BentoDesk";
    ...
    public static DragDropPermissionDiagnostic Diagnose()
    {
        ...
        else if (permissionMismatch)
        {
            ...
            summary = "BentoDesk 正在以管理员权限运行";
            detail = "资源管理器通常是普通权限，管理员权限的 BentoDesk 可能无法接收来自桌面或资源管理器的拖拽。可以使用一键修复清理 BentoDesk 的管理员运行标记、启动项和快捷方式，然后重新启动 BentoDesk。";
        }
        else if (hasAppCompatIssue)
        {
            issue = DragDropDiagnosticIssue.AppCompatIssue;
            summary = "检测到 BentoDesk 兼容性设置异常";
            detail = "兼容模式或“以管理员身份运行”可能会影响 WinUI 3 拖拽。可以使用一键修复清理这些只针对 BentoDesk 的兼容性标记。";
        }
        else if (hasStartupIssue || hasShortcutIssue)
        {
            issue = DragDropDiagnosticIssue.StartupShortcutIssue;
            summary = "检测到启动入口可能指向旧版本";
            detail = "开机启动项或快捷方式可能仍指向旧路径，或者带有异常参数。可以使用一键修复重写当前用户的 BentoDesk 启动入口。";
        }
        else
        {
        ...
        return new DragDropPermissionDiagnostic(
            ...
            summary,
            detail,
            FormatProcessToken("BentoDesk", currentToken),
    ...
    private static string GetCurrentExePath()
    {
        return Environment.ProcessPath
            ?? Path.Combine(AppContext.BaseDirectory, "BentoDesk.exe");
    }
    ...
    private static void AddAppCompatEntries(List<AppCompatEntry> entries, RegistryKey root, string rootName, HashSet<string> relevantPaths)
    {
        try
        {
            ...
            {
                if (!IsRelevantBentoDeskPath(valueName, relevantPaths))
                {
                    continue;
                }
    ...
    private static bool IsRelevantBentoDeskPath(string valueName, HashSet<string> relevantPaths)
    {
        ...
        return string.Equals(fileName, "BentoDesk.exe", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static void CreateOrUpdateShortcut(string shortcutPath, string targetPath, string arguments)
    {
        ...
        shellLink.SetIconLocation(Path.Combine(AppContext.BaseDirectory, "Assets", "bentodesk.ico"), 0);
        var persistFile = (System.Runtime.InteropServices.ComTypes.IPersistFile)shellLink;
    ...
    private static ProcessTokenSnapshot GetCurrentProcessTokenSnapshot()
    {
        ...
        {
            return ProcessTokenSnapshot.Unknown(Environment.ProcessId, "BentoDesk", ex.Message);
        }
    }
