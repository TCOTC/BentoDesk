...
public sealed partial class QuickCaptureWidgetWindow
{
    private void DetailCopyButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        Clipboard.Flush();
        App.Current.QuickCaptureService?.MarkClipboardTextWrittenByBentoDesk(text);
        ShowCopyToast();
    }
    ...
    private async void QuickCaptureAttachmentCopyButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            BentoDeskClipboardWriteScope.MarkWrite(hasImage: true, paths: [attachment.FilePath]);
            Clipboard.SetContent(dataPackage);
            Clipboard.Flush();
            ShowCopyToast();
        }
