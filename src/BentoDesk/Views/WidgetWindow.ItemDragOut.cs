...
public sealed partial class WidgetWindow
{
    ...
    private bool TryPrepareItemDragPackage(DataPackage dataPackage, IReadOnlyList<WidgetItem> draggedItems)
    {
        ...
        dataPackage.Properties["BentoDeskSourceWidgetId"] = ViewModel.Config.Id;
        dataPackage.Properties["BentoDeskSourcePaths"] = sourcePaths;
        dataPackage.Properties["BentoDeskInternalDragToken"] = BentoDeskInternalDragToken;
    ...
    private static bool HasBentoDeskInternalDragData(DataPackagePropertySetView properties)
    {
        return string.Equals(
                   TryGetPackageString(properties, "BentoDeskInternalDragToken"),
                   BentoDeskInternalDragToken,
                   StringComparison.Ordinal) &&
               TryGetPackageStringArray(properties, "BentoDeskSourcePaths").Count > 0;
    }

    private static bool HasPathDropData(DataPackageView dataView)
    {
        return TryGetPackageStringArray(dataView.Properties, "BentoDeskSourcePaths").Count > 0 ||
    ...
    {
        var sourcePaths = TryGetPackageStringArray(dataView.Properties, "BentoDeskSourcePaths")
    ...
    private static bool TryNormalizeUrl(string candidate, out string downloadedPath)
    {
        ...
        try
        {
            ...
            string tempDir = Path.Combine(Path.GetTempPath(), "BentoDesk", "VirtualDrops", Guid.NewGuid().ToString("N"));
