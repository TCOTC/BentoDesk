...
public sealed partial class WidgetWindow
{
        private void CopySelectedPathsToClipboard()
        {
            ...
            BentoDeskClipboardWriteScope.MarkWrite(text: clipboardText, paths: selectedPaths);
        ...
        private async Task CopyImageTextAsync(WidgetItem item)
        {
            ...
            try
            {
                ...
                BentoDeskClipboardWriteScope.MarkWrite(text: ocrResult.Text);
                Clipboard.SetContent(package);
                Clipboard.Flush();
        ...
        private async Task CopySelectionToClipboardAsync(bool cut)
        {
            ...
            BentoDeskClipboardWriteScope.MarkWrite(text: clipboardText, paths: sourcePaths);
            ...
            if (!shellClipboardSet)
            {
                ...
                package.Properties["BentoDeskSourceWidgetId"] = ViewModel.Config.Id;
                package.Properties["BentoDeskSourcePaths"] = sourcePaths;
                package.Properties["BentoDeskInternalDragToken"] = BentoDeskInternalDragToken;
                Clipboard.SetContent(package);
                Clipboard.Flush();
            }
        ...
        private async Task PasteFromClipboardAsync()
        {
            ...
            IReadOnlyList<string> sourcePaths = TryGetPackageStringArray(clipboard.Properties, "BentoDeskSourcePaths")
                .Where(path => !string.IsNullOrWhiteSpace(path) && (File.Exists(path) || Directory.Exists(path)))
                .Select(Path.GetFullPath)
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToArray();
            ...
            if (moveWhenMapped == true)
            {
                await SyncMoveSourceAsync(
                    TryGetPackageString(clipboard.Properties, "BentoDeskSourceWidgetId"),
                    TryGetPackageStringArray(clipboard.Properties, "BentoDeskSourcePaths"));
            }
